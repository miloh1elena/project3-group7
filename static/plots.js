...
async function fetchDataForBarChart() {
  const [sourceData, ratingData, reviewDateData] = await Promise.all([
    fetch('http://localhost:8000/api/v1.0/source').then((res) => res.json()), // Make sure the API endpoint is correct
    fetch('http://localhost:8000/api/v1.0/rating').then((res) => res.json()),
...
fetchDataForBarChart().then(function (data) {
  // Initialize bar chart and dropdown
  createBarChart(data);
